...
const RegisterEstateFormInner = memo(function RegisterEstateFormInner({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: "", location: "", total_plots: "", available_plots: "", price_per_plot: "", status_tag: "" });
      ...
      <div className="panel w-full max-w-xl p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          <div>
            ...
            <input className="field" placeholder="B-Top Garden" value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} />
            {form.name && <p className="mt-1 text- text-muted-foreground">Will save as: <span className="font-medium text-navy">{toTitleCase(form.name)}</span></p>}
          </div>
          <div>
            <label className="label-field">Location</label>
            <input className="field" placeholder="Shimawa, Ogun State" value={form.location} onChange={e => setForm(f => ({...f, location: e.target.value}))} />
            {form.location && <p className="mt-1 text- text-muted-foreground">Will save as: <span className="font-medium text-navy">{toTitleCase(form.location)}</span></p>}
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label-field">Total plots</label><input type="number" className="field" placeholder="420" value={form.total_plots} onChange={e => setForm(f => ({...f, total_plots: e.target.value}))} /></div>
            <div><label className="label-field">Available</label><input type="number" className="field" placeholder="Defaults to total" value={form.available_plots} onChange={e => setForm(f => ({...f, available_plots: e.target.value}))} /></div>
          </div>
          <div><label className="label-field">Price per plot (₦)</label><input type="number" className="field" placeholder="3500000" value={form.price_per_plot} onChange={e => setForm(f => ({...f, price_per_plot: e.target.value}))} /></div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button disabled={saving} className="inline-flex items-center gap-2 rounded bg-navy px-4 py-2 text-sm text-white disabled:opacity-50">{saving && <Loader2 className="h-4 w-4 animate-spin" />}Register</button>
          </div>
        </form>
      </div>
    </div>
...
type Estate = { id: string; name: string; location: string; total_plots: number; available_plots: number; price_per_plot: number; status_tag: string | null; };
...
const EstateCard = memo(function EstateCard({ estate, registeredToday, onRegister }: { estate: Estate; registeredToday: number; onRegister: () => void }) {
  ...
  const pct = estate.total_plots? Math.round((sold / estate.total_plots) * 100) : 0;
  const soldOut = estate.available_plots <= 0;
  return (
    <button type="button" onClick={onRegister} disabled={soldOut} className="panel group w-full p-5 text-left hover:border-navy/30 disabled:opacity-70">
      <div className="flex justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text- font-bold text-navy">{toTitleCase(estate.name)}</h3>
        ...
        <span className="shrink-0 rounded-full bg-amber-100 px-2.5 py-1 text- font-semibold text-amber-800">{pct}% sold</span>
      </div>
      <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-navy" style={{ width: `${Math.min(pct, 100)}%` }} /></div>
      ...
      <div className="mt-3 flex flex-wrap items-end justify-between gap-2">
        <div><div className="text- font-semibold uppercase tracking-wide text-muted-foreground">Price / Plot</div><div className="text- font-bold text-navy">{naira.format(estate.price_per_plot)}</div></div>
      ...
      <div className="mt-3 flex items-center gap-1 text-xs font-semibold text-navy opacity-0 group-hover:opacity-100"><UserPlus className="h-3.5 w-3.5" />{soldOut? "Sold out" : "Register subscriber"}</div>
    </button>
  );
});
...
export function EstatesPage() {
  ...
  const totals = useMemo(() => estates.reduce((a, e) => ({ total: a.total + e.total_plots, available: a.available + e.available_plots }), { total: 0, available: 0 }), [estates]);

  return (
    <div className="w-full space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
        <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-navy-soft text-navy"><Map className="h-5 w-5" /></div><div><h1 className="text-xl font-bold text-navy">Plot Inventory</h1><p className="text-sm text-muted-foreground">{estates.length} estates • {totals.available} of {totals.total} available</p></div></div>
      ...
      {loading? <Skeleton /> : estates.length === 0? <div className="panel p-12 text-center text-sm text-muted-foreground">No estates yet.</div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{estates.map(e => <EstateCard key={e.id} estate={e} registeredToday={counts[e.id]?? 0} onRegister={() => setSubEstate(e)} />)}</div>}
